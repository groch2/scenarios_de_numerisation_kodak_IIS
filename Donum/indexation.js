...
var settingsFile = "C:\\InfoInputSolution\\params.txt";
var GetReferenceAPI = "";
function load(batch) {

  /*var fichierParamétrage = new File(settingsFile);
  var sc = new Scanner(fichierParamétrage);
...
function preProcess(node) {
  ...
  var requete = "select DISTINCT(C.type_doc) from C_CHARTEDOC C, param_coteform P, ref_coteform R where p.code_type_doc = C.code_type_doc and R.code_cote = p.code_cote and R.FORMULAIRE = p.FORMULAIRE order by c.TYPE_DOC";
  var query = dbServer.query(requete);
  fill(node.fields['TYPE_DE_DOCUMENT'], query);

  var requete = "select DISTINCT(R.LIB_COTE) from C_CHARTEDOC C, param_coteform P, ref_coteform R where p.code_type_doc = C.code_type_doc and R.code_cote = p.code_cote and R.FORMULAIRE = p.FORMULAIRE order by R.LIB_COTE";
  var query = dbServer.query(requete);
  fill(node.fields['COTE_DU_DOCUMENT'], query);


  var p1 = node.fields['ChampRecupere'].value.indexOf("|");
  node.fields['COTE_DU_DOCUMENT'].value = node.fields['ChampRecupere'].value.substring(0, p1);
  node.fields['TYPE_DE_DOCUMENT'].value = node.fields['ChampRecupere'].value.substr(p1 + 4);


  //var requete = "select code_redacteur from V_GESTREDAC_PROD order by code_redacteur";
  var requete = "select CONCAT(code_redacteur,' - ',Gestionnaire_redacteur) AS code_redacteur from V_GESTREDAC_PROD order by code_redacteur"
  var query = dbServer.query(requete);
  fill(node.fields['CODE_DU_GESTIONNAIRE'], query);


  GetReferenceAPI = node.fields['GetReferenceAPI'].value;
...
function postProcess(node) {
  ...
  if (node.fields['LIBELLE_DOCUMENT'].value == '' || node.fields['LIBELLE_DOCUMENT'].value == '-') {
...
function fieldFocusLost(field, index) {
  ...
  if (field.name == 'NUMERODECOMPTE' && field.value != '') {
    var requete = "SELECT [NO_CLIENT],[FULL_NAME],[CODE_GEST],[LIB_GEST],[GROUPE] ,[SERVICE] FROM [GEDMAF].[dbo].[V_ALL_ADHERENT] where [NO_Client] =" + field.value;
    var query = dbServer.query(requete);
    if (query.length == 1) {
      node.fields['NOM_ADHERENT'].value = query[0][1];
      node.fields['CODE_DU_GESTIONNAIRE'].value = query[0][2];
    }
  ...
  if (field.name == 'NUMERO_GC') {
    //var url = new URL("https://api-gc-intra.int.maf.local/api/v2/dossier-gc/getByIdOrReference?reference=" + field.value); 
    var url = new URL(GetReferenceAPI + field.value);
    ...
    if (status == 200) { field.markValid(); }
    else { field.markInvalid(); }
  }

  if (field.name == 'TYPE_DE_DOCUMENT' && node.fields['COTE_DU_DOCUMENT'].value != '' && node.fields['LIBELLE_DOCUMENT'].value == '') {
    node.fields['LIBELLE_DOCUMENT'].value = node.fields['COTE_DU_DOCUMENT'].value + " - " + field.value;
  }

  /*if(field.name == 'COTE_DU_DOCUMENT' && node.fields['TYPE_DE_DOCUMENT'].value !='' && node.fields['LIBELLE_DOCUMENT'].value =='')
  {
   node.fields['LIBELLE_DOCUMENT'].value = field.value + " - " + node.fields['TYPE_DE_DOCUMENT'].value  ;
  } */
...
function fieldChanged(field, index) {
  if (field.name == 'COTE_DU_DOCUMENT') {
    /*node.fields['TYPE_DE_DOCUMENT'].clearOptions();*/
    if (node.fields['COTE_DU_DOCUMENT'].value == '') {
      node.fields['TYPE_DE_DOCUMENT'].value = '';
    }
    var requete = "select DISTINCT(C.type_doc) from C_CHARTEDOC C, param_coteform P, ref_coteform R where p.code_type_doc = C.code_type_doc and R.code_cote = p.code_cote and R.FORMULAIRE = p.FORMULAIRE and r.lib_cote = '" + node.fields['COTE_DU_DOCUMENT'].value + "' order by c.TYPE_DOC";
    var query = dbServer.query(requete);
    fill(node.fields['TYPE_DE_DOCUMENT'], query);
  }
